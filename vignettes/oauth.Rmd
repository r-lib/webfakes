...
Here we keep the default values for the parameters deciding whether a refresh token is produced when 
an access token is (`refresh`) and how long the access and refresh tokens are valid (`3600L` and `7200L`, respectively).
...
## OAuth's httr tools, server with refresh or not
...
```{r no-refresh, eval = FALSE}
rsapp <- local_app_process(
  oauth2_resource_app(access_duration = 10L, refresh = FALSE),
  opts = server_opts(num_threads = 3)
)
...
The token can also be used as fake token when you use mocking (with vcr or httptest) in your tests (the fake token could be used on continuous integration for instance to fool your package).
