...
app, and it can be used to record information and then retreive it in
future requests. E.g. here is an end point that fails three times, then
succeeds once, fails again three times, etc.

```{r}
retry <- presser::new_app()
retry$get("/unstable", function(req, res) {
...
```{r}
pr <- presser::new_app_process(retry)
url <- pr$url("/unstable")
httr::GET(url)
httr::GET(url)
httr::GET(url)
