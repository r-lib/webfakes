...
`app$locals` is an environment that belongs to the app, and it can be used to record information and then retreive it in future requests.
E.g. here is an end point that fails three times, then succeeds once, fails again three times, etc.

Note that the `counter` starts at 0, not 1.
