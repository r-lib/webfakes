...
* A web app is just an R object. It can be saved to disk, copied to another
  R process, put in an R package, etc.
* A web app is extensible, by adding new routes and middleware to it.
* Helper functions for sending JSON, files from disk, etc.
* Comes with an app similar to the https://httpbin.org API, so for simple
  tests you don't need to write your own web app.
...
* The digest and jsonlite packages are needed for the `httpbin_app()` app.
